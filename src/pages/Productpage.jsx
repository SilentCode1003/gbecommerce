...
const Productpage = () => {
  ...
  const Product =
    location.state?.product || products.find((Product) => Product.id === id);
  ...
  useEffect(() => {
    const fetchProductInfo = async () => {
      const response = await fetch("/api/products/getproducts/" + id);
      const data = await response.json();

      const { angles, image } = data.data[0];
      setProductInfo(data.data);
      setImageAngles(angles);
      setSelectedImage(image);
    };
    fetchProductInfo();
  }, [productInfo, imageAngles, selectedImage]);
  ...
  const openNotification = () => {
    ...
    api.success({
      ...
      description: `You have added ${quantity} ${productInfo.model} to your quote.`,
  ...
  const renderContent = () => {
    return (
      ...
      >
        {activeTab === "tab1" && <ProductTab1 />}
        {activeTab === "tab2" && <ProductTab2 />}
        {activeTab === "tab3" && <ProductTab3 />}
      </div>
    );
  };
  ...
  return (
    <div className="bg-[#f5f5f5]">
      {contextHolder}
      <Breadcrumb
        className="bg--200 px-10 pt-10 pb-4 text-sm text-gray-400"
        separator=">"
        items={[
          {
            title: <a href="/">Home</a>,
          ...
          {
            title: <a href="">{productInfo.model}</a>,
          },
        ]}
      />
      <div className="mlg:mx-10 md:mx-10 sm:mx-5 xs:mx-5 lg:mx-10 grid grid-cols gap-5">
        <div className="bg-white flex px-7 py-6 flex-col items-center gap-5 lg:flex-row lg:gap-10 lg:items-start justify-center">
          ...
          <div className="flex gap-4 ">
            <div className="flex flex-col gap-4">
              {imageAngles.map((item) => (
                <img
                  ...
                  className={`w-[90px] h-auto rounded-lg border ${
            ...
            <div className="flex items-start justify-start">
              <Image.PreviewGroup
                items={imageAngles} // Ensure Product.images is an array of URLs
                preview={{
                  current: imageAngles.indexOf(selectedImage), // Active image index
                  onChange: (index) => setSelectedImage(imageAngles[index]), // Update selected image on change
                  toolbarRender: (
                    _,
                    {
                      transform: { scale },
          ...
          <div className="w-full lg:w-1/2 max-w-2xl pt-5 px-6">
            ...
            <div className="flex flex-wrap gap-1 items-center">
              ...
              <Tag>{productInfo.description}</Tag>
        ...
        <div className="bg-white h-[30rem] py-2">
          <div className="w-full mx-auto p-4 rounded-lg border-gray-300">
            <div className="flex border-b border-gray-200">
              <button
                ...
                className={`flex-1 py-2 px-4 text-center rounded-t-lg ${
                  activeTab === "tab1"
                    ? "bg-gray-100 border-b-2 border-black font-semibold"
                    : ""
                }`}
              >
                Product Details
            ...
            <div className="p-4 overflow-auto max-h-96">{renderContent()}</div>
          </div>
        </div>
      </div>
      <Footer />
